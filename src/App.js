...
function App() {
  ...
  // --- Compras ---
  const [modalCompra, setModalCompra] = useState({ abierto: false, producto: null, indice: null });
  ...
  const [notificacionCompra, setNotificacionCompra] = useState({ mostrar: false, mensaje: '', producto: null });
  ...
  useEffect(() => {
    // Solo guardar productos de usuario, no los por defecto
    const productosUsuario = productos.filter(prod => prod.creador !== 'MIROPA');
  ...
  const handleLogout = () => {
    ...
    setTimeout(() => {
      ...
      setModalCompra({ abierto: false, producto: null, indice: null });
  ...
  const handleComprar = (idx) => {
    setModalCompra({ abierto: true, producto: productos[idx], indice: idx });
    setCompra({ nombre: '', numero: '', vencimiento: '', cvv: '', direccion: '' });
    setErroresCompra({});
    setCompraExitosa(false);
  };
  ...
  const handleSubmitCompra = (e) => {
    ...
    if (Object.keys(errs).length === 0) {
      // Obtener el producto comprado y su índice
      ...
      setNotificacionCompra({
        mostrar: true,
        mensaje: `¡Compra realizada con éxito! El producto "${productoComprado.nombre}" ha sido eliminado del inventario.`,
        producto: productoComprado
      });
      
      setTimeout(() => {
        cerrarModalCompra();
        setNotificacionCompra({ mostrar: false, mensaje: '', producto: null });
      }, 4000);
    }
  };
        ...
        {notificacionCompra.mostrar && (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                <div style={{ fontWeight: '600', marginBottom: '4px' }}>¡Compra Exitosa!</div>
                <div style={{ fontSize: '0.9rem' }}>{notificacionCompra.mensaje}</div>
                {notificacionCompra.producto && (
                  <div style={{ fontSize: '0.8rem', marginTop: '4px', opacity: '0.9' }}>
                    Precio: {Number(notificacionCompra.producto.precio).toLocaleString('es-CL', { style: 'currency', currency: 'CLP', minimumFractionDigits: 0 })}
                  </div>
                )}
              </div>
            </div>
...
function App() {
  ...
  const handleSubmitCompra = (e) => {
    ...
    if (Object.keys(errs).length === 0) {
      ...
      console.log('Eliminando producto:', productoComprado.nombre, 'en índice:', indiceProducto);
      console.log('Productos antes de eliminar:', productos.length);
      
      // Eliminar el producto comprado (solo si no es un producto por defecto)
      if (indiceProducto !== null && indiceProducto !== -1 && productoComprado.creador !== 'MIROPA') {
        const nuevosProductos = productos.filter((_, i) => i !== indiceProducto);
        console.log('Productos después de eliminar:', nuevosProductos.length);
        setProductos(nuevosProductos);
      }
